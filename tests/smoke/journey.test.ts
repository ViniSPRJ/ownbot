 ...
 *   bun run test:smoke
 *
 * `OWNBOT_API_URL` points it at a deployment on other ports. Without `OWNBOT_SMOKE` the file is
 * skipped, so `bun run test` stays honest on a machine with nothing running.
 */

const asked = ownbotEnv(process.env, "OWNBOT_SMOKE") === "1";
const API = ownbotEnv(process.env, "OWNBOT_API_URL") ?? "http://localhost:3001";
const BOT = ownbotEnv(process.env, "OWNBOT_SMOKE_BOT") ?? "risk-analyst";
...
beforeAll(async () => {
  ...
  if (!reachable) {
    throw new Error(
      `No deployment is answering at ${API}. Start one with \`bash scripts/start.sh\`, or set OWNBOT_API_URL.`,
    );
  }
});
