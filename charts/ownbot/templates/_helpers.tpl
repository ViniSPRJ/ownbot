...
{{- define "ownbot.labels" -}}
helm.sh/chart: {{ include "ownbot.chart" . }}
{{ include "ownbot.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
{{- define "ownbot.selectorLabels" -}}
app.kubernetes.io/name: {{ include "ownbot.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end -}}

{{/* Labels for one component, so two workloads in one release never select each other's pods. */}}
{{- define "ownbot.componentLabels" -}}
{{ include "ownbot.labels" .root }}
app.kubernetes.io/component: {{ .component }}
{{- end -}}

{{- define "ownbot.componentSelectorLabels" -}}
{{ include "ownbot.selectorLabels" .root }}
app.kubernetes.io/component: {{ .component }}
{{- end -}}

{{- define "ownbot.componentName" -}}
{{- printf "%s-%s" (include "ownbot.fullname" .root) .component | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{- define "ownbot.serviceAccountName" -}}
{{- if .Values.serviceAccount.create -}}
{{- default (include "ownbot.fullname" .) .Values.serviceAccount.name -}}
{{- else -}}
...
{{- define "ownbot.secretName" -}}
{{- default (printf "%s-secrets" (include "ownbot.fullname" .)) .Values.secrets.existingSecret -}}
{{- end -}}

{{- define "ownbot.configMapName" -}}
{{- printf "%s-config" (include "ownbot.fullname" .) -}}
{{- end -}}
...
- name: AGENT_COMPUTER_URL
  value: http://{{ include "ownbot.componentName" (dict "root" . "component" "computer") }}-0.{{ include "ownbot.componentName" (dict "root" . "component" "computer") }}:4100
...
- name: COMPUTER_SANDBOX_TEMPLATE_FILE
  value: /etc/ownbot/sandbox-template.json
{{- end }}
{{- /*
...
- name: INTELLIGENCE_API_KEY
  valueFrom:
    secretKeyRef:
      name: {{ include "ownbot.secretName" . }}
...
- name: COMPUTER_TOKEN
  valueFrom:
    secretKeyRef:
      name: {{ default (include "ownbot.secretName" .) .Values.computers.existingTokenSecret }}
      key: computer-token
      optional: {{ eq .Values.computers.mode "external" }}
{{- /*
  One definition, for the same reason `ownbot.databaseUrlEnv` is one (see its comment above): the
...
podAntiAffinity:
  requiredDuringSchedulingIgnoredDuringExecution:
    - topologyKey: kubernetes.io/hostname
      labelSelector:
        matchLabels:
{{ include "ownbot.componentSelectorLabels" (dict "root" $root "component" $component) | indent 10 }}
  ...
  preferredDuringSchedulingIgnoredDuringExecution:
    - weight: 100
      podAffinityTerm:
        ...
        labelSelector:
          matchLabels:
{{ include "ownbot.componentSelectorLabels" (dict "root" $root "component" $component) | indent 12 }}
{{- end -}}
{{- end -}}
...
*/}}
{{- define "ownbot.sandboxPodTemplate" -}}
{{- $spec := dict
  "podTemplate" (dict
