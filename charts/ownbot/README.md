# OwnBot on Kubernetes

Runs OwnBot on any Kubernetes cluster: EKS, GKE, AKS, or your own. One chart, five targets, and the
...
**An image the cluster can pull.** Build and publish this OwnBot fork to a registry you control, then pass `--set image.repository=YOUR_REGISTRY/ownbot --set image.tag=YOUR_TAG`. The default `ownbot` image is a local build name; this rename does not publish an image. Upstream OpenBot images belong to [CopilotKit/OpenBot](https://github.com/CopilotKit/OpenBot) and do not contain the changes in this fork.

**Intelligence credentials.** OwnBot requires CopilotKit Intelligence and the chart refuses to
...
```sh
kubectl create namespace ownbot
kubectl -n ownbot create secret generic ownbot-database \
  --from-literal=database-url='postgresql://USER:PASSWORD@HOST:5432/ownbot?sslmode=require'
```

Then `--set database.existingSecret=ownbot-database`. The key must be `database-url`, or name a
...
kind: ClusterConfig
metadata: { name: ownbot, region: us-east-2, version: "1.34" }
iam: { withOIDC: true }
addons:
  - name: vpc-cni
  - name: coredns
...
```sh
helm dependency build charts/ownbot
helm upgrade --install ownbot charts/ownbot \
  --namespace ownbot --create-namespace \
...
--set database.existingSecret=ownbot-database   # key: database-url
```
...
somebody can try OwnBot in one command.
...
```sh
helm template ownbot charts/ownbot -f charts/ownbot/ci/eks-values.yaml
```
...
OwnBot is a shell for somebody else's agent, and `config.managedAgent.url` is where that agent goes:
