...
**Estado:** não implementado. Não há perfil ACP Hermes no OwnBot, não há mapeamento de agentes e o default nativo do Hermes permanece Qwen3.6 27B. Este documento descreve o menor adaptador seguro da fase seguinte. Não ativa o binário nativo, não enfraquece o guard HTTP e não substitui a fila de filhos do OwnBot.
...
Superfície de ferramentas nativa (além do MCP scoped do OwnBot):
...
- O OwnBot recusa esse `initialize`: `server/src/acp/agent.ts` 158–160 exige `agentCapabilities.mcpCapabilities.http`.
...
4. **Filhos.** `delegate_task` nativo não passa pela fila `message_bot` do OwnBot. A fila e os handoffs continuam a ser do OwnBot (`docs/architecture.md`, `docs/beelink-operations.md`). Um `HERMES_HOME`/registry partilhado exige um filho ACP por tarefa/sessão OwnBot com âmbito, salvo scoping comprovado.
...
- lançar **um processo filho ACP por tarefa/sessão OwnBot com âmbito**, para evitar o compartilhamento de registry entre âmbitos; não reutilizar um único filho ACP entre tarefas/sessões salvo scoping comprovado;
...
- recusar `delegate_task` nativo; filhos só pela fila OwnBot (`message_bot`);
...
8. Sem fuga de credenciais nem do `HOME` do operador. Um filho ACP por tarefa/sessão OwnBot com âmbito, salvo scoping comprovado.
