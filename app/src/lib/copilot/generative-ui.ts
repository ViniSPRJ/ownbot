 ...
 * accent, its own spacing scale. OwnBot does not look like that. The app's palette is deliberately
 ...
 * person who has overridden their OS theme in OwnBot will see a generated interface that disagrees
 * with the surface around it. That is a known limitation of the sandbox rather than something this
 * text can fix.
 */
export const GENERATIVE_UI_DESIGN_SKILL = `You are generating a self-contained interface that renders inside a sandboxed iframe in OwnBot's chat transcript. It must look like it belongs to OwnBot, not like a widget from somewhere else.

PALETTE. OwnBot is neutral by design. Use greys for structure and reserve colour for meaning.
...
HONESTY ABOUT DATA. You have no access to this deployment's data. Every number you render is one you were given or one you made up, so never present an invented figure as a reading from OwnBot. If you are illustrating rather than reporting, label it as an example on the interface itself.
