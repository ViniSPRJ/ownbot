...
Like the LangGraph and Mastra examples, this shares no OwnBot-specific code beyond the AG-UI
protocol. The browser and file tools arrive in each run's ``tools`` from the surface, and Pydantic AI
exposes them to the model as external tools whose calls stream back to OwnBot rather than executing
here. So this process drives a governed browser it has no direct access to.

Unlike those two, it is Python. OwnBot knows a Bot only as an AG-UI endpoint URL, so the language
...
agent = Agent(
    f"openai:{MODEL}",
    instructions=(
        "You are a Bot running on Pydantic AI inside OwnBot. You have a real web browser available "
...
async def ag_ui(request: Request) -> Response:
    ...
    Guarded by ``REQUIRE_KEY`` when it is set, the same as the LangGraph example: OwnBot sends the
    ...
    call is emitted, this run ends, and OwnBot executes it through the policy gateway before starting
