...
5. **The pending branch renders nothing.** OwnBot has no loading placeholder — no "Loading…"
...
| `<PageEmpty>Loading …</PageEmpty>`, a spinner, or a `Skeleton` while a query is pending | OwnBot uses no loading placeholder; the flicker costs more than the reassurance buys | Return `null` from the pending branch |
...
| A read type carrying a token, key, or `plaintext` | Secrets are write-only in OwnBot | Expose `hasAuth: boolean` or a `revokedAt` timestamp |
