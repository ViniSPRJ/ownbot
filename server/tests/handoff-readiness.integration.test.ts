...
test.skipIf(!process.env.OPERATIONS_READONLY_TEST_DATABASE_URL)(
  ...
  async () => {
    const db = createDatabase(
      process.env.OPERATIONS_READONLY_TEST_DATABASE_URL!,
      { max: 1 },
    );
    try {
      ...
      for (const fixture of [
        ...
        {
          name: "unresolved admitted effect",
          kind: "bot.message",
          age: 30,
          attempts: 1,
          lease: null,
        ...
        const [result] = await db.execute(sql`
          WITH work_items AS (SELECT ${"unknown" in fixture ? sql`jsonb_build_object('result', jsonb_build_object('outcome', 'unknown'))` : sql`'{}'::jsonb`} AS payload, ${fixture.kind}::text AS kind,
