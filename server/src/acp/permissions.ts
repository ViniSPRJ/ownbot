...
export class AcpPermissionGate {
  ...
  private tool(call: ObjectValue, permission = false): string | undefined {
    ...
    if (this.provider === "codex") {
      if (
        ...
        call.rawInput?.server !== "ownbot"
      )
        return;
      name = call.rawInput.tool;
    } else if (this.provider === "claude") {
      const qualified = permission
        ? call.name
        : call._meta?.claudeCode?.toolName;
      if (
        typeof qualified !== "string" ||
        !qualified.startsWith("mcp__ownbot__")
      )
        return;
      name = qualified.slice("mcp__ownbot__".length);
    } else if (this.provider === "pi") {
      if (
        call._meta?.ownbot_pi_tool !== true ||
        call.rawInput?.server !== "ownbot"
      )
        return;
      name = call.rawInput.tool;
      ...
      if (
        input?.variant !== "MCPTool" ||
        typeof input.tool_name !== "string" ||
        !input.tool_name.startsWith("ownbot__")
      )
        return;
      name = input.tool_name.slice("ownbot__".length);
    }
    return typeof name === "string" && this.granted.has(name)
      ? name
      : undefined;
  }
  ...
  observe(value: unknown): void {
    ...
    if (
      !call ||
      !["tool_call", "tool_call_update"].includes(call.sessionUpdate)
    )
      return;
  ...
  decide(value: unknown, sessionId: string | undefined): object {
    ...
    const option = Array.isArray(request.options)
      ? request.options.find(
          (x: unknown) =>
            object(x)?.kind === "allow_once" &&
            typeof object(x)?.optionId === "string",
        )
      : undefined;
