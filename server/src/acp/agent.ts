...
export class AcpAgent extends AbstractAgent {
  ...
  constructor(
    private readonly options: {
      ...
      prompt: string;
      profile: AcpProfile;
      tools: (input: RunAgentInput) => Promise<readonly GrantedTool[]>;
  ...
  clone(): AcpAgent {
    return new AcpAgent(this.options);
  }
  abortRun(): void {
    this.stop?.();
  }
  run(input: RunAgentInput): Observable<BaseEvent> {
    return new Observable((subscriber) => {
      ...
      const emit = (event: Record<string, unknown>) => {
        if (!subscriber.closed) subscriber.next(event as BaseEvent);
      };
      const endText = () => {
      ...
      const stop = () => {
        ...
        if (!subscriber.closed)
          subscriber.error(new Error("Execução ACP cancelada."));
      };
      this.stop = stop;
      void (async () => {
        if (running.has(key))
          throw new Error("Este agente já está trabalhando nesta conversa.");
        running.add(key);
        ownsLock = true;
        /*
        ...
        const persist = async (value: AcpSessionState) =>
          writeSessionState(stateFile, value);
        try {
          const chosen =
            (await o.resolveModel?.(input.threadId).catch(() => null)) ?? null;
          selection = {
            ...selection,
            model: chosen ?? o.profile.model ?? null,
          };
          ...
          try {
            ...
            const parsed = parseSessionState(raw, selection);
            saved = parsed.state;
          ...
          phase = "tools";
          const { tools, guidance: toolGuidance } = acpToolsForProvider(
            o.profile.provider ?? "codex",
            await o.tools(input),
          );
          const permissions = new AcpPermissionGate(
            o.profile.provider ?? "codex",
          ...
          transport = new AcpStdioTransport({
            ...
            onNotification: (method, value) => {
              ...
              const event = value as {
                ...
                update?: {
                  toolCallId?: string;
                  rawInput?: { server?: string; tool?: string };
                  _meta?: {
              ...
              if (
                update?.toolCallId &&
                !seenToolCalls.has(update.toolCallId) &&
          ...
          const clientCapabilities =
            (o.profile.provider ?? "codex") === "codex"
              ? {
                  _meta: {
                    jetbrains: {
          ...
          const init = await transport.request<{
            ...
            protocolVersion: 1,
            clientCapabilities,
            clientInfo: { name: "ownbot", version: "0.1.0" },
          });
          if (init.protocolVersion !== 1)
            throw new Error("Versão ACP não suportada");
          ...
          const plan = planSessionAnchor({
            ...
            restartRequested: o.restart === true,
          });
          ...
          if (plan.load && plan.sessionId) {
            try {
              sessionConfiguration = await transport.request("session/load", {
               ...
               */
              if (!(error instanceof AcpRpcError) && !(error instanceof Error))
                throw error;
              ...
              await persist({
                ...
                ...(saved?.replyMessageId
                  ? { replyMessageId: saved.replyMessageId }
                  : {}),
                replyMessageIds: saved?.replyMessageIds ?? [],
          ...
          if (!resumed) {
            const session = await transport.request<{
              ...
              configOptions?: unknown;
              models?: unknown;
            }>("session/new", { cwd, mcpServers: [bridge.descriptor] });
          ...
          const previousReplies = plan.previousReplyIds;
          ...
          const messages = input.messages
            ...
            .filter(
              (m) =>
                m.role !== "system" &&
                !(fromIndex > 0 && previousReplies.has(m.id)),
            );
          ...
          accepting = true;
          phase = "prompt";
          const result = await transport.request<{ stopReason: string }>(
            "session/prompt",
            {
              sessionId,
              prompt: [
          ...
          if (result.stopReason !== "end_turn")
            throw new Error("A CLI não concluiu o turno.");
          if (cancelled) return;
          if (!textStarted || !messageId)
            throw new Error(
          ...
          if (ownsLock) running.delete(key);
          transport?.close();
          await bridge?.close();
        ...
        console.warn(
          JSON.stringify({
            ...
            phase,
            stopReason,
            cause: acpFailureCause(error),
          }),
        );
        if (!cancelled)
          subscriber.error(
            new Error(
              "A execução ACP não foi concluída. Verifique autenticação, perfil e disponibilidade da CLI; não houve fallback para API.",
            ),
          );
      });
      return stop;
