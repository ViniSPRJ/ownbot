...
export async function createToolBridge(
  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      if (
        presented.length !== authorization.length ||
        !timingSafeEqual(presented, authorization)
      ) {
      ...
      switch (body.method) {
        case "initialize":
          return result({
            ...
            capabilities: { tools: {} },
            serverInfo: { name: "ownbot", version: "1.0.0" },
          });
        case "ping":
          return result({});
        case "tools/list":
        ...
        case "tools/call": {
          ...
          const validated = await tool.parameters.safeParseAsync(
            params.arguments ?? {},
          );
          ...
          try {
            return result({
              content: [
                { type: "text", text: await tool.execute(validated.data) },
              ],
            });
          } catch {
